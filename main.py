...
class DatabaseManager:
    def __init__(self):
        ...
        self.db_config = {
            ...
            'user': os.getenv('DB_USER', 'your_password'),
    ...
    def find_user_by_email(self, email):
        ...
        try:
            ...
            query = "SELECT id, email, reset_password_code, reset_password_code_expiry FROM user WHERE email = %s"
            cursor.execute(query, (email,))
            user = cursor.fetchone()
            logger.debug(f"Result: {email}: {'found' if user else 'didnt found'}")
            return user
        except mysql.connector.Error as e:
            logger.error(f"Error: {e}")
            return None
...
class EmailSender:
    def __init__(self):
        ...
        self.config = {
            ...
            'password': os.getenv('SMTP_PASSWORD', 'your_smtp_email'),
